...
import trimesh
import csv
#from utils.pc_utils import rotate_point_cloud_by_axis_angle, sample_point_cloud_by_n
...
SPLIT_CSV_PATH = "/home/M2021-CJX/ml_hw/Multimodal-Shape-Completion-master/data/shapenet-official-split.csv"
...
class EPNdataset(Dataset):
    def __init__(self,config):
        super(EPNdataset, self).__init__()
        ...
        phase=self.phase = config.subset
        self.aug = self.phase == "train"
    ...
    def __getitem__(self, index):
        ...
        raw_pc = sample_point_cloud_by_n(raw_pc, self.n_pts)
        ...
        #fake_pc = real_pc[torch.randperm(real_pc.size(0))]
        fake_pc = real_pc[np.random.permutation(self.n_pts)]
        drop_rate = random.randint(1, self.n_pts)
        fake = np.random.permutation(drop_rate)
        fake_pc = fake_pc[fake]
        #fake_pc = fake_pc[drop_rate:, :]

        fake_pc=upsample_point_cloud(fake_pc,self.n_pts)

        rand_choice = random.randint(0, 1)
        fake_pc = torch.tensor(fake_pc, dtype=torch.float32)
        if rand_choice==1:
            fake_pc=raw_pc

        return real_shape_name,raw_pc_name,(real_pc,raw_pc,fake_pc)
        #{"raw": raw_pc, "real": real_pc, "raw_id": raw_pc_name, "real_id": real_shape_name}
