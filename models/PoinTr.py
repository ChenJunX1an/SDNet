...
from .Transformer_raw import PCTransformer
...
class PoinTr(nn.Module):
    ...
    def get_loss(self, ret, gt,partial):
        gt=gt[2]
        loss_coarse = self.loss_func(ret[0], gt)
        loss_fine = self.loss_func(ret[1], gt)
        loss_all = (loss_fine+loss_coarse) * 1e3
        losses = [loss_coarse,loss_coarse,loss_fine,loss_fine]
        return loss_all, losses, gt
