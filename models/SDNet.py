...
class UpLayer(nn.Module):
    ...
    def __init__(self, dim, seed_dim, up_factor=2, i=0, radius=1, n_knn=20, interpolate='three', attn_channel=True):
        ...
        self.mlp_2 = MLP_CONV(in_channel=128 * 4, layer_dims=[256, dim])
...
class SDNet(nn.Module):
    def __init__(self, config, **kwargs):
        ...
        self.mlp_1 = MLP_CONV(in_channel=3, layer_dims=[64, 128])
        self.fuse_mlp1=MLP_CONV(in_channel=256+128, layer_dims=[256, 128])
    ...
    def get_loss(self, pcds_pred, gt,partial):
        ...
        loss_all = (cd1 + cd2+cd3+cd4+cd5) #* 1e3  # +torch.sum(torch.stack(delta_losses)) / 3
