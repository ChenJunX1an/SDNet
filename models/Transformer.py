...
class PCTransformer(nn.Module):
    ...
    def forward(self, inpc,cov_feat):
        '''
        ...
        bs = inpc.size(0)
        #Npoints=inpc.size(1)
        #print(cov_feats.size())
        #inpcs = torch.cat([cov_feats,inpc],dim = -1)
        ...
        cov_feats = cov_feat.unsqueeze(1).expand([-1, self.num_query, -1])
        query_covfeature=torch.cat([cov_feats, query_feature], dim=-1)
        q = self.mlp_query(query_covfeature.transpose(1,2)).transpose(1,2) # B M C
        # decoder
